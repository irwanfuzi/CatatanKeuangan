...
class AnalisisScreen extends StatelessWidget {
        ...
        builder: (context, snapshot) {
          ...
          for (var item in riwayat) {
            if (item['jenis'].toString().toLowerCase().contains('pengeluaran')) {
              String namaKategori = item['kategori'] ?? 'Lainnya';
              double nominal = (item['nominal'] ?? 0).toDouble();
              kategoriMap[namaKategori] = (kategoriMap[namaKategori] ?? 0) + nominal;
              totalPengeluaranAngka += nominal;
            }
          }

          List<Color> chartColors = [Colors.redAccent, Colors.blueAccent, Colors.orangeAccent, Colors.greenAccent, Colors.purpleAccent];
          int colorIndex = 0;
          ...
          if (totalPengeluaranAngka > 0) {
            kategoriMap.forEach((kategori, jumlah) {
              ...
              pieSections.add(PieChartSectionData(color: chartColors[colorIndex % chartColors.length], value: persentase, title: '${persentase.toStringAsFixed(0)}%', radius: 50, titleStyle: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.white)));
              colorIndex++;
            });
          } else {
            pieSections.add(PieChartSectionData(color: Colors.grey.shade300, value: 100, title: '0%', radius: 50));
          }
          ...
          return ListView(
            ...
            children: [
              ...
              Row(
                children: [
                  Expanded(child: _buildSummaryBox('Pemasukan', pemasukan, FontAwesomeIcons.arrowDown, Colors.green)),
                  const SizedBox(width: 12),
                  Expanded(child: _buildSummaryBox('Pengeluaran', pengeluaran, FontAwesomeIcons.arrowUp, Colors.red)),
                ],
              ),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 24),
                    SizedBox(height: 200, child: PieChart(PieChartData(sectionsSpace: 2, centerSpaceRadius: 40, sections: pieSections))),
                  ],
                ),
              ),
              const SizedBox(height: 24),
                ...
                return Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text('Rp ${entry.value.toInt()}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w900, color: Colors.red)),
                    ],
                  ),
                );
              }).toList(),
  ...
  Widget _buildSummaryBox(String title, String amount, IconData icon, Color color) {
    return Container(
      padding: const EdgeInsets.all(16),
